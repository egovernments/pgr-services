...
class ViewDCB extends Component {
  ...
  initData() {
    ...
    console.log(query);
    Api.commonApiPost(url, query, {}, false, specifications[`dcb.view`].useTimestamp).then(function(res){
  ...
  componentDidMount() {
    ...
    var getDemands = {
      consumerCode: decodeURIComponent(this.props.match.params.searchParam) + 
      '&businessService=PT&receiptRequired=true'
    }
    
    Api.commonApiPost('/billing-service/demand/_search',getDemands,{},false,true).then((res)=>{
      this.setState({
  ...
  render() {
    ...
    return (
      <div className="Report">
        <form id="printable">
        {!_.isEmpty(mockData) && mockData["dcb.view"] && <ShowFields groups={mockData["dcb.view"].groups} noCols={mockData["dcb.view"].numCols} ui="google" handler={""} getVal={getVal} fieldErrors={fieldErrors} useTimestamp={mockData["dcb.view"].useTimestamp || false} addNewCard={""} removeCard={""} screen="view"/>}
                ...
                <Table style={{color:"black",fontWeight: "normal", marginBottom:0, minWidth:'100%', width:'auto'}}  bordered responsive>
                  <thead>
                    <tr>
                      ...
                      <th  colSpan="2" style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.demand")}</th>
                      <th colSpan="2" style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.fields.collection")}</th>
                      <th colSpan="2" style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.fields.balance")}</th>
                    </tr>
                    <tr>
                      <th></th>
...
class HeadwiseDCB extends Component {
  ...
  printData = (itemName, item) => {
    if(itemName == 'demand') {
      item.demandDetails.map((unit) => { return <td>{unit.taxAmount}</td>})
    }
    else if(itemName == 'collection') {
      item.demandDetails.map((unit) => { return <td>{unit.collectionAmount}</td>})
    }
  }
      ...
      <div>
        <div>
          <div style={{textAlign:'center', paddingTop:10}}> 
            <RaisedButton type="button" primary={true} label={translate('pt.create.button.viewdcb.headwisedcb')} style={{margin:'0 5px'}}
              onClick= {this.popupHeadwiseDCB.bind(this)}
            />
          </div>
          {this.state.showHeadWiseDCB ? 
          <div className="headwiseDCBTable" style={{ position: "fixed", /* Stay in place */
            zIndex: 2, /* Sit on top */
            left: 0,
            top: 0,
            height: "100%",
            ...
            <Card className="uiCard">
              <CardTitle   title={<div style={{color:"#354f57", fontSize:18, margin:'8px 0'}}>{translate("pt.create.button.viewdcb.headwisedcb")}</div>}/>
              <CardText>
                <Grid fluid>
                  { demandsData && demandsData.length ? 
                  <Table style={{color:"black",fontWeight: "normal", marginBottom:0, minWidth:'100%', width:'auto'}}  bordered responsive>
                    <thead>
                      <tr>
                        <th style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.fields.installment")}</th>
                        <th  colSpan={demandsData[0].demandDetails.length} style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.demand")}</th>
                        <th colSpan={demandsData[0].demandDetails.length} style={{textAlign:'center'}}>{translate("pt.create.groups.addDemand.fields.collection")}</th>
                      </tr>
                      <tr>
                        <th></th> 
                        {demandsData[0].demandDetails.map((item) => {
                          return(
                            <th>{item.taxHeadMasterCode}</th>
                          )
                        })}
                        {demandsData[0].demandDetails.map((item) => {
                          return(
                    ...
                    <tbody>
                      {demandsData.map((item)=> {
                        return( <tr>
                          <td>{new Date(item.taxPeriodFrom).getFullYear()+ " - " + (new Date(item.taxPeriodTo).getFullYear()) }</td>
                          {item.demandDetails.map((obj) => { return(<td>{obj.taxAmount}</td>)})}
                          {item.demandDetails.map((obj) => { return(<td>{obj.collectionAmount}</td>)})}
                        </tr>)
                      })}
                    </tbody>
                  </Table>
                ...
                <div style={{textAlign:'center', paddingTop:10}}> 
                  <RaisedButton type="button" primary={true} label={translate('pt.create.button.viewdcb.close')} style={{margin:'0 5px'}}
...
class ReceiptTable extends Component {
  ...
  totalReceiptAmount() {
    var total = 0;
    this.props.receiptData.map((item) => {(total = total+item.receiptAmount)})
    return total;
  }
  render() {
    ...
    return(
      <div>
        <Card className="uiCard">
          <CardTitle   title={<div style={{color:"#354f57", fontSize:18,margin:'8px 0'}}>{translate("collection.search.categorytype.collectedReceipts")}</div>}/>
          <CardText>
            <Grid fluid>
              <Table style={{color:"black",fontWeight: "normal", marginBottom:0, minWidth:'100%', width:'auto'}}  bordered responsive>
                <thead>
