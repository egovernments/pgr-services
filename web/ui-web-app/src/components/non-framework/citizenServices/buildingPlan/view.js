...
class Report extends Component {
  ...
  initData() {
    ...
    let self = this;
    let obj = specifications["fn.view"];
    ...
    //Get view form data
    var url = specifications["fn.view"].url.split("?")[0];
        ...
        fileUpload(documents[i], "wc", function(err, res) {
          ...
          if(err) {
            ...
            _docs.push({
              ...
              newFile: (self.state.role != "CITIZEN" && ServiceRequest.status == "APPROVED" ? true : null)
            })
                              ...
                              <Table id="ReceiptForWcAPartTwo" responsive bordered condensed>
                                  <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <td colSpan={4}>
                                          {self.state.Receipt[0].instrument.instrumentType.name == "Cash" || self.state.Receipt[0].instrument.instrumentType.name == "Online" ? "NA" : self.state.Receipt[0].instrument.bank.name}
                                        </td>
                                      </tr>
                                  </tbody>
                              </Table>
