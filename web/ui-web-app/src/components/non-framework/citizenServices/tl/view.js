...
class Report extends Component {
  ...
  initData() {
    ...
    if(self.props.match.params.status == "pay") {
      ...
      Api.commonApiPost("/citizen-services/v1/requests/_search", {consumerCode: decodeURIComponent(self.props.match.params.ackNo)}, {}, null, true).then(function(res2) {
          ...
          Api.commonApiPost("/tl-masters/category/v1/_search", {"type": "subcategory"}, {}, null, true).then(function(res3) {
  ...
  makePayment = (res, fee) => {
   ...
   Api.commonApiPost("/citizen-services/v1/pgrequest/_create", {}, {PGRequest}, null, self.props.metaData["tl.create"].useTimestamp, false, null, JSON.parse(localStorage.userRequest)).then(function(res){
       self.props.setLoadingStatus('hide');
                      ...
                      <CardText style={{paddingTop:0,paddingBottom:0}}>
                        <Grid style={{paddingTop:0}}>
                          <Row>
                            ...
                            <Col xs={12} md={6}>
                              <input type="file" multiple style={{"marginTop":"40px"}} onChange={(e) => {
                                    self.setState({
                              ...
                              <Table id="ReceiptForWcAPartTwo" responsive bordered condensed>
                                  <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <td colSpan={4}>
                                          {self.state.Receipt[0].instrument.instrumentType.name == "Online" || self.state.Receipt[0].instrument.instrumentType.name == "Cash" ? "NA" : self.state.Receipt[0].instrument.bank.name}
                                        </td>
                                      </tr>
                                  </tbody>
                              </Table>
