...
getVal = (path,isDate) => {
  ...
  render() {
    ...
    const renderFiles = function() {
      {return formData && formData.licenses && formData.licenses[0] && formData.licenses[0].supportDocuments && formData.licenses[0].supportDocuments.length && formData.licenses[0].supportDocuments.map(function(v, i) {
        return (
          <tr key={i}>
            <td>{i+1}</td>
        ...
        const renderDocuments = function() {
          ...
          const renderBody = function() {
            ...
            if(formData && formData.hasOwnProperty("licenses") && formData.licenses && formData.licenses[0].feeDetails && formData.licenses[0].feeDetails.length>0){
    ...
    const renderAgreement = function() {
      if(formData && formData.hasOwnProperty("licenses") && formData.licenses.length>0){
        ...
        if(formData.licenses[0].isPropertyOwner){
          return (
            <Card className="uiCard">
                  <CardHeader style={styles.reducePadding}  title={<div style={{color:"#354f57", fontSize:18,margin:'8px 0'}}>{translate("tl.view.licenses.groups.agreementDetails")}</div>} />
